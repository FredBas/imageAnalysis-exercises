...
def process_rgb_image(img, counter):
    strength = math.sin(counter / 10) * 10  # oscillates between -10 and +10
    radius = 200 + (counter % 200)  # vary radius over time
    center = (img.shape[1] // 2, img.shape[0] // 2)  # swirl around image center

    return swirl(img, strength=strength, radius=radius, center=center)
